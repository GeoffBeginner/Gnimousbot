  ...
  if (stats.isDirectory()) {
    fs.readdirSync(scripts).forEach((file) => {
      try {
        ...
        if (config) {
          const {
            name = [], role = '0', version = '1.0.0', hasPrefix = true, aliases = [], description = '', usage = '', credits = '', cooldown = '1'
														...
														if (event.body !== null) {
															...
															const downloadAndSendFBContent = async (url) => {
																try {
																	...
																	return api.sendMessage({ body: "𝖠𝗎𝗍𝗈 𝖣𝗈𝗐𝗇 𝖥𝖺𝖼𝖾𝖻𝗈𝗈𝗄 𝖵𝗂𝖽𝖾𝗈\n\n GNIMOUS BOT 𝟭.𝟬.𝟬𝘃", attachment: fs.createReadStream(fbvid) }, event.threadID, () => fs.unlinkSync(fbvid));
																}
																catch (e) {
